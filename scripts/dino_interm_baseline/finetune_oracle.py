...
from dino import *
from inference_server_simulate_dino import convert_base_to_moe
...
def adapt(args):
    random.seed(42)
    desc_train, desc_valid = 'oracle_train%.2f_cocotrain' % args.train_r, 'oracle_%.2fvalid' % (1 - args.train_r)
...
def evaluate_all_videos(args):
    ...
    for im in images_valid:
        ...
        im['file_name'] = os.path.join(images_base_dir, im['video_id'], 'unmasked', im['file_name'])
    loader = torchdata.DataLoader(EvaluationDataset(cfg, images_valid), batch_size=None, collate_fn=EvaluationDataset.collate, shuffle=False, num_workers=args.num_workers)
...
if __name__ == '__main__':
    ...
    assert args.train_r >= 0.1 and args.train_r <= 0.9
