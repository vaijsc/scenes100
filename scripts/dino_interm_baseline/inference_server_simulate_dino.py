...
def convert_base_to_moe(state_dict, cfg, interm=True):
    if not interm:    
        moe_state_dict = {}
        for key in state_dict.keys():
            if '.bbox_embed.' in key or '.class_embed.' in key:
                ...
                for branch_id in range(cfg.MODEL.ADAPTIVE_BUDGET):
                    new_key =  key[:dot_position] + f".{branch_id}" + key[dot_position:]
        ...
        moe_state_dict = {}
        for key in state_dict.keys():
            if 'model.backbone.0.body.layer3.0.conv1' in key or 'model.backbone.0.body.layer3.0.conv2' in key:
                for branch_id in range(cfg.MODEL.ADAPTIVE_BUDGET):
                    new_key = key[:36] + f'.experts.{branch_id}' + key[36:]
                    moe_state_dict[new_key] = deepcopy(state_dict[key])    
            else:
                moe_state_dict[key] = deepcopy(state_dict[key])  
...
class AdaptativePartialTrainer(DefaultTrainer):
    def __init__(self, cfg, incremental_videos, ckpt):
        ...
        if ckpt is not None:
            ...
            model.load_state_dict(convert_base_to_moe(state_dict, cfg, interm=True), strict=True)
        ...
        budget = cfg.MODEL.ADAPTIVE_BUDGET
        cfg.MODEL.ADAPTIVE_BUDGET = 1
        self.model_teacher = self.build_model(cfg)
        teacher_state_dict = torch.load("../../models/dino_5scale_remap_orig.pth")['model']
        self.model_teacher.load_state_dict(convert_base_to_moe(teacher_state_dict, cfg, interm=True), strict=True)
        self.model_teacher.eval()
        cfg.MODEL.ADAPTIVE_BUDGET = budget
