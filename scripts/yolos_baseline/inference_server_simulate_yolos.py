...
from yolos import *
from config import add_yolos_config
...
class AdaptativePartialTrainer(DefaultTrainer):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        if hasattr(plain_model.backbone, 'no_weight_decay'):
            skip = plain_model.backbone.no_weight_decay()
        head = []
        backbone_decay = []
        backbone_no_decay = []
        ...
        param_dicts = [
            {"params": head},
            {"params": backbone_no_decay, "weight_decay": 0., "lr": cfg.SOLVER.BASE_LR},
            {"params": backbone_decay, "lr": cfg.SOLVER.BASE_LR},
        ]
