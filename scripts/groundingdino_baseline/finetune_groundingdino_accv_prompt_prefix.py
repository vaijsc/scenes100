...
def prepend(text_dict, prompt_len):
    padding = torch.zeros((1, prompt_len), device='cuda:0')
    # Concatenate the zeros with the original tokenized_input_ids
    text_dict['tokenized_input_ids'] = torch.cat((padding, text_dict['tokenized_input_ids']), dim=1)
    
    text_dict['tokenized_type_ids'] = torch.cat((padding, text_dict['tokenized_type_ids']), dim=1)

    D = text_dict['encoded_text'].size(2)  # Get the dimension D
    # Generate random values from a uniform distribution in range [-0.5, 0.5]
    new_columns = -0.5 + torch.rand(1, prompt_len, D, device='cuda:0')
    # Split the original tensor into two parts: the first column and the rest
    first_column = text_dict['encoded_text'][:, :1, :]
    remaining_columns = text_dict['encoded_text'][:, 1:, :]
    # Concatenate the tensors: first column, new columns, and remaining columns
    text_dict['encoded_text'] = torch.cat((first_column, new_columns, remaining_columns), dim=1)

    trues_to_insert = torch.ones((1, prompt_len), dtype=torch.bool, device='cuda:0')
    # Concatenate the new True values with the original text_token_mask
    text_dict['text_token_mask'] = torch.cat((trues_to_insert, text_dict['text_token_mask']), dim=1)

    pos_ids_insert = torch.arange(prompt_len, device='cuda:0').unsqueeze(0)  # Shape (1, num_elements_to_insert)
    # Split the original tensor into the first element and the remaining elements
    first_element = text_dict['position_ids'][:, :1]  # Shape (1, 1)
    remaining_elements = text_dict['position_ids'][:, 1:]  # Shape (1, N - 1)
    # Concatenate the tensors: first element, sequence to insert, and remaining elements
    text_dict['position_ids'] = torch.cat((first_element, pos_ids_insert, remaining_elements), dim=1)

    P = prompt_len  # Number of tokens to insert

    # Get the original size (N) of the caption (number of tokens)
    original_size = text_dict['text_self_attention_masks'].size(1)  # N

    # Create a new self_attention_mask tensor with space for the P inserted tokens
    new_size = original_size + P
    new_self_attention_mask = torch.zeros((1, new_size, new_size), dtype=torch.bool, device='cuda:0')

    # Fill the new mask by copying over the old mask
    # First, copy the original attention mask to the appropriate area in the new mask
    new_self_attention_mask[:, 1+P :, 1+P :] = text_dict['text_self_attention_masks'][:, 1:, 1:]

    # Insert the new tokens
    for i in range(1, P + 1):
        # Inserted tokens can attend to all tokens except the first and last
        new_self_attention_mask[:, i, 1:-1] = True
        new_self_attention_mask[:, 1:-1, i] = True

    # The first and last token can only attend to themselves
    new_self_attention_mask[:, 0, :] = False
    new_self_attention_mask[:, -1, :] = False
    new_self_attention_mask[:, :, 0] = False
    new_self_attention_mask[:, :, -1] = False
    new_self_attention_mask[:, 0, 0] = True
    new_self_attention_mask[:, -1, -1] = True

    text_dict['text_self_attention_masks'] = new_self_attention_mask
...
def train_few_shot(args):
    ...
    if args.arch == 'lora':
        ...
        # Prompt tuning insert

        text_dict_cache = prepend(text_dict_cache, args.prompt_len)
        # Init from encoded text
        prefix_prompt = text_dict_cache['encoded_text'][:, 1 : args.prompt_len + 1, :].detach().clone()
        prefix_prompt.requires_grad = True
        if args.ckpt is not None:
            print('loading from', args.ckpt)
            prefix_prompt = torch.load(args.ckpt)
            text_dict_cache['encoded_text'][:, 1 : args.prompt_len + 1, :] = prefix_prompt

        optimizer = torch.optim.Adam([prefix_prompt], lr=args.lr)
        print(f"Prompt params take {prefix_prompt.numel()/num_model_params*100:.2f}%")
        ...
        for it in tqdm.tqdm(range(0, args.iters), ascii=True):
            ...
            for im_torch, targets in zip(im_torch_batch, targets_batch):
                if args.arch == 'prompt':
                    ...
                    text_dict_copy['encoded_text'][:, 1 : args.prompt_len + 1, :] = prefix_prompt
            ...
            if it in eval_iters:
                if args.arch == 'lora':
                    ...
                    torch.save(prefix_prompt, prefix + '.pth')
                elif args.arch == 'enhancer':
...
def eval_fewshot(args):
    ...
    if args.arch == 'lora':
        ...
        text_dict = get_text_dict(model, args.prompt)
        text_dict = prepend(text_dict, args.prompt_len)
        prefix_prompt = torch.load(args.ckpt)
        text_dict['encoded_text'][:, 1 : args.prompt_len + 1, :] = prefix_prompt
    elif args.arch == 'enhancer':
        model.enhancer = PromptEnhancer(args.enhancer_r_qkv, args.enhancer_r_ff).cuda()
        model.enhancer.eval()
        model.enhancer.load_state_dict(torch.load(args.ckpt))
...
def show_detected(args):
    ...
    outputdir = '/mnt/f/intersections_results/accv24/gdino/case_study'
    for video_id in images_per_video:
        with contextlib.redirect_stdout(open(os.devnull, 'w')):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prompt_len', type=int, default=5, help='Prompt prefix tuning length')
