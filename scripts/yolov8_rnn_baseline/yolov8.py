...
def visualize_feat(feature_map, input_image, filename):
    # feature map shape C x H x W
    # input image shape 3 x H x W
    H_input, W_input = input_image.shape[1], input_image.shape[2]
    aggregated_map = feature_map.mean(dim=0)  # Shape: H x W
    normalized_map = (aggregated_map - aggregated_map.min()) / (aggregated_map.max() - aggregated_map.min())
    
    resized_map = F.interpolate(normalized_map.unsqueeze(0).unsqueeze(0), size=(H_input, W_input), mode='bilinear', align_corners=False)
    resized_map = resized_map.squeeze()  # Shape: H_input x W_input
    
    heatmap = cv2.applyColorMap((resized_map.detach().cpu().numpy() * 255).astype('uint8'), cv2.COLORMAP_JET)
    input_image = (input_image.squeeze().permute(1, 2, 0).cpu().numpy() * 255).astype('uint8')  # Convert to HxWx3 uint8 format
...
class DetectionModel(BaseModel):
    ...
    @staticmethod
    def draw(image_tensor, detection_labels, format='yolo', input=True, ratio=None, file_name="test_preprocess.png", thing_classes=None):
        import matplotlib.pyplot as plt
        import matplotlib.patches as patches
        import numpy as np
        # Convert the PyTorch tensor to a NumPy array for visualization
        if isinstance(image_tensor, np.ndarray):
            image_np = image_tensor.transpose(1, 2, 0)
        else:
            image_tensor = image_tensor.cpu()
            image_np = image_tensor.permute(1, 2, 0).numpy()
        
        # Find the bounding box of non-zero regions
        non_zero_rows = np.any(image_np != 0, axis=(1, 2))  # Find rows with non-zero pixels
        non_zero_cols = np.any(image_np != 0, axis=(0, 2))  # Find columns with non-zero pixels

        row_min, row_max = np.where(non_zero_rows)[0][[0, -1]]  # Get min and max row indices
        col_min, col_max = np.where(non_zero_cols)[0][[0, -1]]  # Get min and max column indices

        # Crop the image to remove zero padding
        image_np = image_np[row_min:row_max+1, col_min:col_max+1, :]
        ...
        if format == 'yolo':
            ...
            assert ratio is not None, "ratio unavailable"
            pred_bboxes = detection_labels.pred_boxes.tensor.cpu()
            pred_classes = detection_labels.pred_classes.cpu()
            for i, bbox in enumerate(pred_bboxes):
                ...
                if thing_classes is not None:
                    ...
                    ax.text(x1, y1 - 5, class_name, color='white', fontsize=max(5, int((y2-y1) * 0.04)))
                rect = patches.Rectangle((x1, y1), x2 - x1, y2 - y1, linewidth=1, edgecolor='r', facecolor='none')
                ax.add_patch(rect)
    ...
    def preprocess_image(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        for i, im in enumerate(batched_inputs):
            if self.training:
                ...
                # DetectionModel.draw(images[i], instances[:, 1:], file_name=f"input_afterpreprocess_{i}.png")   
