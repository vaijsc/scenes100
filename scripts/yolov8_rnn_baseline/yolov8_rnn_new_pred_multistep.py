...
from FastFlowNet.demo import find_flow_and_warp, find_flow, warp
...
class FeaturePrediction(torch.nn.Module):
    def __init__(self, in_channels):
        super(FeaturePrediction, self).__init__()
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=1)
        self.conv.weight.data = torch.eye(in_channels).view(in_channels, in_channels, 1, 1)
        self.conv.bias.data.zero_()
    def forward(self, x):
...
class TemporalEnhancer(torch.nn.Module):
    def __init__(self, feature_dim):
        super(TemporalEnhancer, self).__init__()
        # Linear layers for projecting into Q, K, and V spaces
        self.query_proj = nn.Linear(feature_dim, feature_dim)
        self.key_proj = nn.Linear(feature_dim, feature_dim)
        self.value_proj = nn.Linear(feature_dim, feature_dim)
        self.softmax = nn.Softmax(dim=-1)
        
        # Initialize the weights to favor the current frame feature initially
        self._initialize_weights()

    def _initialize_weights(self):
        # Initialize key and value projections with small values
        nn.init.normal_(self.key_proj.weight, mean=0.0, std=1e-5)
        nn.init.normal_(self.value_proj.weight, mean=0.0, std=1e-5)
        
        # Optionally, initialize query projection to identity (so it doesn't change the current frame feature initially)
        nn.init.eye_(self.query_proj.weight)
        nn.init.zeros_(self.query_proj.bias)

    def forward(self, cumulative_feature, current_frame_feature):
        # Reshape inputs (C, W, H) -> (C, W*H) for matrix multiplication
        _, C, W, H = cumulative_feature.shape
        cumulative_flat = cumulative_feature.clone().view(C, -1)  # (C, W*H)
        current_flat = current_frame_feature.view(C, -1)  # (C, W*H)

        # Project to Q, K, V
        Q = self.query_proj(current_flat.T)  # (W*H, C)
        K = self.key_proj(cumulative_flat.T)  # (W*H, C)
        V = self.value_proj(cumulative_flat.T)  # (W*H, C)

        # Compute attention scores (W*H, C) * (C, W*H) -> (W*H, W*H)
        attention_scores = torch.matmul(Q, K.T)  # (W*H, W*H)
        
        # Softmax to get attention weights
        attention_weights = self.softmax(attention_scores)  # (W*H, W*H)
        
        # Apply attention to values
        attended_values = torch.matmul(attention_weights, V)  # (W*H, C)

        # Add attended values to current frame feature
        attended_values = attended_values.T.view(1, C, W, H)  # reshape back to (C, W, H)
        fused_feature = attended_values + current_frame_feature
        # fused_feature = attended_values + cumulative_feature
...
class YOLORNN(DetectionModel):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        for i in range(len(batched_inputs)):
            visualize_feat(y[15][i], batch['img'][i], f'feature_vis/orig_small_{i}.png')
            visualize_feat(y[18][i], batch['img'][i], f'feature_vis/orig_middle_{i}.png')
            visualize_feat(y[21][i], batch['img'][i], f'feature_vis/orig_large_{i}.png')
        ...
        enhanced_feats_prev = list(tensor.clone().requires_grad_(True) for tensor in enhanced_feats)
        # (1xC1xH1xW1, 1xC2xH2xW2, 1xC3xH3xW3)
        ...
        for t in range(1, batch['img'].shape[0]): # t from 1 to B-1
            last_hidden = self.h[-1] # (1xC1xH1xW1, 1xC2xH2xW2, 1xC3xH3xW3)
            visualize_feat(last_hidden[0][0], batch['img'][t], f'feature_vis/hidden_small_{t}.png')
            visualize_feat(last_hidden[1][0], batch['img'][t], f'feature_vis/hidden_middle_{t}.png')
            visualize_feat(last_hidden[2][0], batch['img'][t], f'feature_vis/hidden_large_{t}.png')
            # breakpoint()
            enhanced_feats =  [self.temp_enhancers[i](h, f[t: t+1]) for i, (h, f) in enumerate(zip(last_hidden, all_feats))]
            # (1xC1xH1xW1, 1xC2xH2xW2, 1xC3xH3xW3)
            visualize_feat(enhanced_feats[0][0], batch['img'][t], f'feature_vis/enhanced_small_{t}.png')
            visualize_feat(enhanced_feats[1][0], batch['img'][t], f'feature_vis/enhanced_middle_{t}.png')
            visualize_feat(enhanced_feats[2][0], batch['img'][t], f'feature_vis/enhanced_large_{t}.png')
            
            enhanced_feats_clone = list(tensor.clone().requires_grad_(True) for tensor in enhanced_feats)
            enhanced_feats_clone_2 = list(tensor.clone().requires_grad_(True) for tensor in enhanced_feats)
            ...
            # Calculate P(t) = H(F(G(x(t-1), w(t-1))))
            h_prev = self.h[-2] 
            flow = find_flow(self.flow_model, batch['img'][t-1:t], batch['img'][t:t+1])
            warped_feats = [warp(self.flow_model, enhanced_feats_prev[i], flow) for i in range(len(enhanced_feats_prev))]
            # p_t_prev = self.model[-1](warped_feats)
            
            visualize_feat(warped_feats[0][0], batch['img'][t], f'feature_vis/pred_small_{t}.png')
            visualize_feat(warped_feats[1][0], batch['img'][t], f'feature_vis/pred_middle_{t}.png')
            visualize_feat(warped_feats[2][0], batch['img'][t], f'feature_vis/pred_large_{t}.png')
            ...
            mask_gt = batch['batch_idx'] == t
            K_gt = mask_gt.sum().item() 
            gt_t = {'batch_idx': torch.zeros(K_gt).cuda(), 'cls': batch['cls'][mask_gt].view(-1, 1), 'bboxes': batch['bboxes'][mask_gt].view(-1, 4)}
            
            # inner_consistency_loss, inner_consistency_loss_components = self.criterion(d_t_train, targets_o_t)
            inner_consistency_loss, inner_consistency_loss_components = self.criterion(d_t_train, gt_t)
            # inner_pred_loss, inner_pred_loss_components = self.criterion(p_t_prev, targets_o_t)
            # inner_pred_loss, inner_pred_loss_components = self.criterion(p_t_prev, gt_t)

            mse_loss = torch.nn.MSELoss()
            inner_pred_loss = 0
            for i in range(3):
                inner_pred_loss += mse_loss(warped_feats[i], all_feats[i][t:t+1])
            # inner_pred_loss, inner_pred_loss_components = (warped_feats, [all_feats[i][t:t+1] for i in range(3)])
            
            inner_loss = inner_consistency_loss + inner_pred_loss #+ sum([torch.linalg.norm(self.h[-1][i]) for i in range(len(self.h[-1]))]) # * alpha
            for i in tqdm.tqdm(range(self.num_step)): # num step
                self.inner_optimizer.zero_grad()
                inner_loss.backward(retain_graph=True)
                self.inner_optimizer.step()

            visualize_feat(self.h[-1][0].grad[0], batch['img'][t], f'feature_vis/hidden_small_grad_{t}.png')
            visualize_feat(self.h[-1][1].grad[0], batch['img'][t], f'feature_vis/hidden_middle_grad_{t}.png')
            visualize_feat(self.h[-1][2].grad[0], batch['img'][t], f'feature_vis/hidden_large_grad_{t}.png')

            self.h[-1] = [warp(self.flow_model, self.h[-1][i], flow) for i in range(len(self.h[-1]))]

            hidden_state_copy = list(tensor.detach().clone().requires_grad_(False) for tensor in self.h[-1])
            self.h.insert(-1, hidden_state_copy)

            enhanced_feats_prev = enhanced_feats_clone_2
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        with torch.enable_grad():
            ...
            for t in range(1, batch['img'].shape[0]): # t from 1 to B-1
                ...
                visualize_feat(self.h[-1][2].grad[0], batch['img'][t], f'feature_vis/hidden_large_grad_{t}.png')
                breakpoint()
                self.h[-1] = [warp(self.flow_model, self.h[-1][i], flow).detach().requires_grad_(True) for i in range(len(self.h[-1]))]
                self.inner_optimizer = torch.optim.Adam(self.h[-1], lr=self.agg_lr)
                breakpoint()
                hidden_state_copy = list(tensor.detach().clone().requires_grad_(False) for tensor in self.h[-1])
                self.h.insert(-1, hidden_state_copy)
                ...
                enhanced_feats_prev = enhanced_feats_clone_2
                # Compute P(t)
                # new_enhanced_feats =  [self.temp_enhancers[i](self.feat_preds[i](h), f[t: t+1]) for i, (h, f) in enumerate(zip(self.h[-1], all_feats))]
                # self.model[-1].training = True
    ...
    @classmethod
    def create_from_sup(cls, net, agg_lr, thing_classes=None, num_step=1):
        net.temp_enhancers = nn.ModuleList([TemporalEnhancer(in_channels).cuda() for in_channels in [128, 256, 512]])
        net.feat_preds = nn.ModuleList([FeaturePrediction(in_channels).cuda() for in_channels in [128, 256, 512]])
        net.flow_model = FastFlowNet().cuda().eval()
        net.agg_lr = agg_lr
        net.thing_classes = thing_classes
        net.num_step = num_step
        # Uncomment if hidden state is learnable
        # self.h = ...
        net.__class__ = cls
        return net
...
class AdaptativePartialTrainer(DefaultTrainer):
    ...
    def __init__(self, cfg, train_whole, incremental_videos, ckpt, full_ckpt):
        ...
        model = YOLORNN.create_from_sup(model, cfg.SOLVER.AGG_LR, cfg.DATASETS.THING_CLASSES, cfg.MODEL.NUM_STEP)
        ...
        if train_whole:
            ...
            trainable_modules = [model.temp_enhancers, model.feat_preds, model.flow_model]
...
def adapt(args):
    ...
    cfg.MODEL.NUM_STEP = args.num_step
