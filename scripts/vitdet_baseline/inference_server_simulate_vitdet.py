...
from vitdet import load_vitdet
...
def adapt(args):
    ...
    # validation images
    dst_cocovalid = get_coco_dicts(_args, "valid")
    desc_manual_valid, dst_manual_valid = 'coco_manual', []
    ...
    dst_pseudo_anno, desc_pseudo_anno = [], 'coco_train'
    ...
    prefix = 'adaptive_partial_server_%s_anno_%s.%s' % ("vit", desc_pseudo_anno, args.tag)

    del _tensor, _args
    gc.collect()
    DatasetCatalog.register(desc_manual_valid, lambda: dst_cocovalid)
    MetadataCatalog.get(desc_manual_valid).thing_classes = thing_classes
    DatasetCatalog.register(desc_pseudo_anno, lambda: dst_cocotrain)
...
python inference_server_simulate_yolov8.py --train_whole 1 --opt adapt --model r101-fpn-3x --ckpt ../../models/yolov8s_remap.pth --clustering 1 --update_cluster_interval 10005 --buffer_size 40 --incremental_videos 1 --tag seq.cluster.budget1 --budget 10 --iters 180000 --eval_interval 2000 --save_interval 2000 --image_batch_size 6 --num_workers 4 --p_replay 0.75 --outputdir ./yolov8_vanilla
'''
