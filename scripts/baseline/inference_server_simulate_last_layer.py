...
class GeneralizedRCNNServer(detectron2.modeling.meta_arch.rcnn.GeneralizedRCNN):
    def get_training_assignment(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        # logic code for random assignment
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        # FPN
        feature_stem = [self.stem_list[m](images.tensor[i : i + 1]) for i, m in enumerate(module_indices)]
        feature_stem = torch.cat(feature_stem, dim=0)
        features = self.backbone(feature_stem, module_indices)
    ...
    def create_from_sup(cls, net, budget):
        ...
        net.stem_list = torch.nn.ModuleList([
            copy.deepcopy(net.backbone.bottom_up.stem) for _ in range(0, net.budget)
        ])

        net.res5_list = torch.nn.ModuleList([
            copy.deepcopy(net.backbone.bottom_up.res5) for _ in range(0, net.budget)
        ])
        # delete stem and res5 stage in the original backbone (replace by Identity module)
        del net.backbone.bottom_up.stem
        
        # net.backbone.bottom_up.res5 = net.res5_list
        # net.backbone.bottom_up.stages[net.backbone.bottom_up.stage_names.index('res5')] = net.res5_list
    
        assert isinstance(net.backbone.bottom_up, detectron2.modeling.backbone.resnet.ResNet)
        net.backbone.__class__ = FPNCluster
        net.backbone.bottom_up.__class__ = ResNetNoStemRes5
...
class GeneralizedRCNNServerCluster(GeneralizedRCNNServer):
    ...
    def update_clusters(self):
        # calculate the loss for each pair (video, path) to find the path for each video
        # save the used and unused paths
        ...
        for i_module in tqdm.tqdm(range(0, self.budget), ascii=True, desc='calculating for each module'):
            for v in self.video_input_buffer:
                ...
                for im in self.video_input_buffer[v]:
                    with torch.no_grad():
                        images = self.preprocess_image([im])
                        gt_instances = [im['instances'].to(self.device)]
                        feature_stem = self.stem_list[i_module](images.tensor)
                        features = self.backbone(feature_stem, [i_module])
                        ...
                        del box_features
                        _losses_list.append(float(sum(loss_dict.values()).item()))
...
class ResNetNoStemRes5(detectron2.modeling.backbone.resnet.ResNet):
    def forward(self, x, module_indices):
        assert x.dim() == 4
        outputs = {}
        for name, stage in zip(self.stage_names, self.stages):
            breakpoint() 
            if name == 'res5':
                x = [stage[m](x[i:i+1]) for i, m in enumerate(module_indices)]
                x = torch.cat(x, dim=0)
            else:
                x = stage(x)
...
class FPNCluster(detectron2.modeling.backbone.fpn.FPN):
    def forward(self, x , module_indices):
        """
        Args:
        ...
        Returns:
            dict[str->Tensor]:
                ...
                ["p2", "p3", ..., "p6"].
        """
        bottom_up_features = self.bottom_up(x, module_indices)
        results = []
...
def adapt(args):
    ...
    for k in tqdm.tqdm(list(state_dict.keys()), ascii=True, desc='populating weights for sub-modules'):
        if k.startswith('backbone.bottom_up.stem.'):
            for i in range(0, args.budget):
                state_dict['stem_list.%d.%s' % (i, k[24:])] = copy.deepcopy(state_dict[k])
        if k.startswith('backbone.bottom_up.res5.'):
            for i in range(0, args.budget):
                state_dict['res5_list.%d.%s' % (i, k[24:])] = copy.deepcopy(state_dict[k])
    ...
    for k in state_dict.keys():
        if k.startswith('backbone.bottom_up.stem.') or k.startswith('proposal_generator.') or k.startswith('roi_heads.box_predictor.'):
            keys_del.append(k)
    for k in keys_del:
        del state_dict[k]
    breakpoint()
    trainer.model.load_state_dict(state_dict)
    trainer.model.backbone.bottom_up.res5 = copy.deepcopy(trainer.model.res5_list)
    trainer.model.backbone.bottom_up.stages[trainer.model.backbone.bottom_up.stage_names.index('res5')] = copy.deepcopy(trainer.model.res5_list)
    del state_dict
    breakpoint()
    gc.collect()
