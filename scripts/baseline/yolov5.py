...
class DetectionModel(BaseModel):
    ...
    def preprocess_image(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        # print("Convert time for image: ", time.perf_counter() - start)
        # start = time.perf_counter()
        for i, im in enumerate(batched_inputs):
            if self.training:
                ...
                # DetectionModel.draw(images[i], instances[:, 1:], file_name="input_afterprepocess.png")   
                # breakpoint()
        # print("Convert time for label: ", time.perf_counter()-start)  

        # breakpoint()  

        if self.training:
            targets = torch.cat([im['instances'].cuda() for im in batched_inputs], dim=0).cuda()
            return images.cuda(), targets
        else:
            return images.cuda()
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]], augment=False, profile=False, visualize=False):
        ...
        conf_thres = 0.4 if self.output_format == 'yolo' else 0.25
        ...
        if self.output_format == 'yolo':            
            ...
            # DetectionModel.draw(x[0], reversed_outputs[0], input=False, ratio=ratio)
            # breakpoint()
            return reversed_outputs
