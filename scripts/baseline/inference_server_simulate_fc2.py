            ...
            for v in self.video_input_buffer:
                ...
                for im in self.video_input_buffer[v]:
                    with torch.no_grad():
                        ...
                        del box_features
                        category_boxes = [[] for c in range(len(thing_classes))]
                        patch_size = (7, 7)
                        for c in range(len(thing_classes)):
                            category_boxes[c].extend([im['instances'].gt_boxes.tensor[i] for i in range(len(im['instances'].gt_classes)) if im['instances'].gt_classes[i].cpu().numpy() == c])
                            if len(category_boxes[c]) > 0:
                                category_boxes[c] = torch.stack(category_boxes[c])
                                patches[c].append(roi_pool(input=features['p5'], boxes=[category_boxes[c]] , output_size=patch_size, spatial_scale=features['p5'].shape[-2]/height))
                        _losses_list.append(float(sum(loss_dict.values()).item()))
                for c in range(len(thing_classes)):
                    patches[c] = torch.cat(patches[c], dim=0)
                feature_video_index[(v, i_module)] = [torch.mean(patches[c], dim=0) for c in range(len(thing_classes))]
        ...
        while len(video_index_loss_set) > 0:
            ...
            if count > self.budget:
                def feature_dist(f1, f2):
                    assert len(f1[0].shape) == 3 and len(f2[0].shape) == 3, "wrong shape"
                    assert f1[0].shape == f1[1].shape, "wrong shape"
                    
                    l2_distance = torch.sqrt((f1[0] - f2[0]).pow(2).sum()) + torch.sqrt((f1[1] - f2[1]).pow(2).sum())
                    return l2_distance
                distance_matrix = {}
