...
class GeneralizedRCNNServerCluster(GeneralizedRCNNServer):
    ...
    def update_clusters(self):
        ...
        for i_module in tqdm.tqdm(range(0, self.budget), ascii=True, desc='calculating for each module'):
            for v in self.video_input_buffer:
                ...
                # average_feature = torch.mean(torch.stack(video_features), dim=0)
    ...
    def get_training_assignment(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        for i, im in enumerate(batched_inputs):
            if im['video_id'] not in sample_mapping:
                sample_mapping[im['video_id']] = [im]
            else:
                sample_mapping[im['video_id']].append(im)
        module_mapping = {}

        module_indices = []
        for idx in sample_mapping:
            if idx == 'coco':
                module_mapping[idx] = np.random.choice(np.arange(0, self.budget))
            elif idx in self.video_id_to_index:
                module_mapping[idx] = self.video_id_to_index[idx]
            else: # first seen during training
                loss_set = []
                for i_module in range(0, self.budget):
                    with torch.no_grad():
                        images = self.preprocess_image(sample_mapping[idx])
                        gt_instances = [im['instances'].to(self.device) for im in sample_mapping[idx]]
                        feature_res2 = self.stem_res2_list[i_module](images.tensor)
                        features = self.backbone(feature_res2)
                        proposals, loss_dict = self.proposal_generator_list[i_module](images, {f: features[f] for f in features}, gt_instances)
                        proposals_roi_sampled = self.roi_heads.label_and_sample_proposals(proposals, gt_instances)
                        ...
                        loss_dict.update(self.box_predictor_list[i_module].losses(predictions, proposals_roi_sampled))
                        del box_features
                        loss_set.append((i_module, float(sum(loss_dict.values()).item())))
                def custom_sort_key(pair):
                    return (-1.0 * pair[1], random.random())
                
                loss_set.sort(key=custom_sort_key)
                # index_to_video_id = {i: [] for i in range(0, self.budget)}
                index_to_video_id = {}
                for v, i in self.video_id_to_index.items():
                    if not i in index_to_video_id:
                ...
                num_vid_encountered = len([j for j in self.video_input_buffer if j != idx]) + 1
                if num_vid_encountered > self.budget:
                    _cap_per_module = num_vid_encountered / self.budget * self.cluster_cap_factor
                else:
                    _cap_per_module = 0.5 # when number of buffered videos <= budget, enforce 1 to 1 mapping
                while len(loss_set) > 0:
                    i_module, _ = loss_set.pop()
                    # for mapped_videos in index_to_video_id.values():
                    #     assert v not in mapped_videos, '%s %s' % (v, index_to_video_id)
                    if i_module in index_to_video_id and len(index_to_video_id[i_module]) >= _cap_per_module:
                        continue
                    if i_module in index_to_video_id:
                        index_to_video_id[i_module].append(idx)
                    else:
                        index_to_video_id[i_module] = [idx]
                    
                    self.video_id_to_index[idx] = i_module
                    self.used_indices[i_module] = True
                    if i_module in self.un_used_indices:
                        del self.un_used_indices[i_module]
                    break
                ...
                print('buffer count:', {v: len(self.video_input_buffer[v]) for v in self.video_input_buffer})
                print(f'mapping (id {idx} first seen in traning):')
                for i in sorted(list(index_to_video_id.keys())):
                    print(i, sorted(index_to_video_id[i]))
                print('unused:', self.un_used_indices)
        ...
        for im in batched_inputs:
            module_indices.append(module_mapping[im['video_id']])
