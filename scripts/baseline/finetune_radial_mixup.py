...
finetune_output = os.path.join(os.path.dirname(__file__), 'finetune_output_radial_mixup')
...
class RadialTransform(torch.nn.Module):
    # (cx, cy) is the center of distortion. image center is (0, 0). bottom-right corner is (1, 1). scales of cx/cy are different!
    # r is the radius of distortion. half-edge is 1. rx/ry always to scale of w/h
    # px, py are the strength of distortion. 1: no distortion. <1: barrel distortion. >1: pin-cushion distortion
    def __init__(self, cx, cy, r, px, py):
        super(RadialTransform, self).__init__()
        # print(cx, cy, r, px, py); plt.figure(figsize=(16, 8));
        # xs, ys = np.meshgrid(np.arange(-1, 1, 0.05), np.arange(-1, 1, 0.05))
        # rs = ((np.stack([xs, ys], axis=2) - np.array([cx, cy]).reshape(1, 1, 2)) ** 2).sum(axis=2) ** 0.5
        # xs_d = np.minimum(rs, (rs / r) ** (1 / px) * r) * (xs - cx) / rs + cx
        # ys_d = np.minimum(rs, (rs / r) ** (1 / py) * r) * (ys - cy) / rs + cy
        # plt.subplot(1, 2, 1); plt.scatter(xs_d.reshape(-1), ys_d.reshape(-1), marker='+', c='b')
        # plt.xlim(-1, 1); plt.ylim(-1, 1); plt.grid();
        # rs = ((np.stack([xs_d, ys_d], axis=2) - np.array([cx, cy]).reshape(1, 1, 2)) ** 2).sum(axis=2) ** 0.5
        # xs_r = np.maximum(rs, (rs / r) ** px * r) * (xs_d - cx) / rs + cx
        # ys_r = np.maximum(rs, (rs / r) ** py * r) * (ys_d - cy) / rs + cy
        # plt.subplot(1, 2, 2); plt.scatter(xs_r.reshape(-1), ys_r.reshape(-1), marker='.', c='r')
        # plt.xlim(-1, 1); plt.ylim(-1, 1); plt.grid();
        # plt.show()

        # fix cx/cy in range (-1, 1), cx = sigmoid(cx_logit) * 2 - 1
        assert min(cx, cy) > -0.99 and max(cx, cy) < 0.99
        self.center_logit = torch.nn.Parameter(torch.FloatTensor([np.log((cx + 1) / (1 - cx)), np.log((cy + 1) / (1 - cy))]).float())

        # fix r in range (0.3, 1), r = sigmoid(r_logit) * 0.7 + 0.3
        assert r > 0.31 and r < 0.99
        self.r_logit = torch.nn.Parameter(torch.tensor(np.log((r - 0.3) / (1.0 - r))).float())

        # fix px/py in range (0.3, 0.9), px = sigmoid(px_logit) * 0.6 + 0.3
        assert min(px, py) > 0.31 and max(px, py) < 0.89
        self.pxy_logit = torch.nn.Parameter(torch.tensor([np.log((_x - 0.3) / (0.9 - _x)) for _x in [px, py]]).float())

        self.zero_scalar = torch.nn.Parameter(torch.tensor(0.0).float())

    def _get_params(self):
        return (torch.sigmoid(self.center_logit) * 2 - 1).view(1, 1, 2), torch.sigmoid(self.r_logit) * 0.7 + 0.3, (torch.sigmoid(self.pxy_logit) * 0.6 + 0.3).view(1, 1, 2)

    def get_cxcyrpxpy(self):
        center, r, pxy = self._get_params()
        return tuple(map(float, [center[0, 0, 0], center[0, 0, 1], r, pxy[0, 0, 0], pxy[0, 0, 1]]))

    def forward(self):
        raise NotImplementedError

    def warp(self, x: torch.Tensor, reverse=False):
        _device = self.r_logit.device
        center, r, pxy = self._get_params()
        # _params = np.array(list(map(float, center[0, 0])) + [float(r)] + list(map(float, pxy[0, 0]))); print(_params)
        # if np.isnan(_params).sum() > 0: raise NotImplementedError
        B, C, H, W = x.size()
        xys = torch.stack(torch.meshgrid(
            torch.arange(-W // 2, W // 2, device=_device).float() / (W / 2),
            torch.arange(-H // 2, H // 2, device=_device).float() / (H / 2)
        ), dim=2) # W x H x 2
        # radius coordinates. very small r causes numerical issues!
        rs = torch.sqrt(torch.square(xys - center).sum(dim=2, keepdims=True))
        cos_sin = torch.where(rs > 1e-4, (xys - center) / rs, self.zero_scalar.detach())
        if not reverse:
            xys_distort = torch.min(rs, torch.pow(rs / r, 1.0 / pxy) * r) * cos_sin + center
        else:
            xys_distort = torch.max(rs, torch.pow(rs / r,       pxy) * r) * cos_sin + center
        radial_grid = xys_distort.permute(1, 0, 2).unsqueeze(0).expand(B, -1, -1, -1)
        x_tf = torch.nn.functional.grid_sample(x, radial_grid, mode='bilinear', align_corners=True, padding_mode='zeros')
        x_tf[torch.isnan(x_tf)] = 0
        return x_tf

    def unwarp(self, x: torch.Tensor):
        return self.warp(x, reverse=True)
...
class GeneralizedRCNNFinetuneRadial(detectron2.modeling.meta_arch.rcnn.GeneralizedRCNN):
    def aggregate_warped_features(self, images_tensor):
        ...
        for tf in self.radials:
            ...
            # plt.imshow(_im); plt.title('cxy=(%.2f, %.2f) r=%.2f pxy=(%.2f, %.2f)' % tf.get_cxcyrpxpy())
            # plt.subplot(2, 3, 2)
        ...
        for k in features_tf_dict:
            assert features_tf_dict[k].size(1) == len(self.radials) + 1
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        # print(self.backbone.bottom_up.stem.conv1.weight.data[0, 0, 0, 0].item(), self.roi_heads.box_head.fc1.weight.data[0, 0].item(), self.radials[0].sxsy[0].item())
    ...
    def create_from_sup(net):
        ...
        net.radials = torch.nn.ModuleList([
            # if set center to (0, 0) training will diverge.
            RadialTransform(.01, .01, 0.75, 0.65, 0.65).to(net.device),
            RadialTransform(0.4, -.3, 0.6,  0.65, 0.65).to(net.device),
            RadialTransform(-.4, -.3, 0.6,  0.65, 0.65).to(net.device),
        ])
        net.aggregators = torch.nn.ModuleDict({
            'p2': WeightedFeatureMap(256, len(net.radials) + 1).to(net.device),
            'p3': WeightedFeatureMap(256, len(net.radials) + 1).to(net.device),
            'p4': WeightedFeatureMap(256, len(net.radials) + 1).to(net.device),
            'p5': WeightedFeatureMap(256, len(net.radials) + 1).to(net.device),
            'p6': WeightedFeatureMap(256, len(net.radials) + 1).to(net.device),
        })
        net.__class__ = GeneralizedRCNNFinetuneRadial
        return net


# wrap detectron2/engine/defaults.py:DefaultTrainer
class FinetuneRadialTrainer(DefaultTrainer):
    def __init__(self, cfg, is_base_model):
        ...
        model = GeneralizedRCNNFinetuneRadial.create_from_sup(model)
...
def radial_base_trainer_simple_trainer_run_step(self):
    assert self.model.training, '[SimpleTrainer] model was changed to eval mode!'
    if self.iter < 2:
        print('freeze Faster RCNN parameters, only train aggregators')
        for m in [self.faster_rcnn_radial_model.backbone, self.faster_rcnn_radial_model.proposal_generator, self.faster_rcnn_radial_model.roi_heads, self.faster_rcnn_radial_model.radials]:
            for p in m.parameters():
                p.requires_grad = False
    if self.iter == self.iters_aggr_base_pretrain:
        print('train whole network')
        for p in self.faster_rcnn_radial_model.parameters():
    ...
    self.radial_history.append({'iter': self.iter, 'cxcyrpxpy': [_aggr.get_cxcyrpxpy() for _aggr in self.faster_rcnn_radial_model.radials]})


def radial_finetune_simple_trainer_run_step(self):
    assert self.model.training, '[SimpleTrainer] model was changed to eval mode!'
    start = time.perf_counter()
    data = next(self._data_loader_iter)
    data_time = time.perf_counter() - start
...
def train_base(args):
    ...
    trainer = FinetuneRadialTrainer(cfg, is_base_model=True)
    assert isinstance(trainer._trainer, SimpleTrainer), 'trainer class mismatch'
    trainer._trainer.run_step = types.MethodType(radial_base_trainer_simple_trainer_run_step, trainer._trainer)
    ...
    prefix = 'mscoco2017_remap_radial_%s' % args.model
    results_0 = None
    ...
    with open(os.path.join(args.outputdir, prefix + '.json'), 'w') as fp:
        json.dump({'results': trainer.eval_results_all, 'args': vars(args), 'lr_history': trainer._trainer.lr_history, 'loss_history': trainer._trainer.loss_history, 'radial_history': trainer._trainer.radial_history}, fp)
    m = trainer.model
    ...
    aps, lr_history, loss_history, radial_history = trainer.eval_results_all, trainer._trainer.lr_history, trainer._trainer.loss_history, trainer._trainer.radial_history
    ...
    radial_history_iter_list = np.array([_h['iter'] for _h in radial_history])
    radial_history = np.array([_h['cxcyrpxpy'] for _h in radial_history])
    ...
    plt.subplot(6, 2, 9)
    for r_i in range(0, radial_history.shape[1]):
        plt.plot(radial_history_iter_list, radial_history[:, r_i, 0], linestyle='-', color=colors[r_i])
    plt.legend([('R%d' % r_i) for r_i in range(0, radial_history.shape[1])])
    plt.grid(True)
    plt.xlim(max(iter_list) * -0.02, max(iter_list) * 1.02)
    plt.ylim(radial_history[:, :, :2].min() - 0.01, radial_history[:, :, :2].max() + 0.01)
    plt.xlabel('Training Iterations')
    plt.title('cx')
    plt.subplot(6, 2, 11)
    for r_i in range(0, radial_history.shape[1]):
        plt.plot(radial_history_iter_list, radial_history[:, r_i, 1], linestyle='-', color=colors[r_i])
    plt.legend([('R%d' % r_i) for r_i in range(0, radial_history.shape[1])])
    plt.grid(True)
    plt.xlim(max(iter_list) * -0.02, max(iter_list) * 1.02)
    plt.ylim(radial_history[:, :, :2].min() - 0.01, radial_history[:, :, :2].max() + 0.01)
    ...
    plt.subplot(6, 2, 8)
    for r_i in range(0, radial_history.shape[1]):
        plt.plot(radial_history_iter_list, radial_history[:, r_i, 2], linestyle='-', color=colors[r_i])
    plt.legend([('R%d' % r_i) for r_i in range(0, radial_history.shape[1])])
    plt.grid(True)
    plt.xlim(max(iter_list) * -0.02, max(iter_list) * 1.02)
    plt.ylim(radial_history[:, :, 2].min() - 0.01, radial_history[:, :, 2].max() + 0.01)
    ...
    plt.subplot(6, 2, 10)
    for r_i in range(0, radial_history.shape[1]):
        plt.plot(radial_history_iter_list, radial_history[:, r_i, 3], linestyle='-', color=colors[r_i])
    plt.legend([('R%d' % r_i) for r_i in range(0, radial_history.shape[1])])
    plt.grid(True)
    plt.xlim(max(iter_list) * -0.02, max(iter_list) * 1.02)
    plt.ylim(radial_history[:, :, -2:].min() - 0.01, radial_history[:, :, -2:].max() + 0.01)
    ...
    plt.subplot(6, 2, 12)
    for r_i in range(0, radial_history.shape[1]):
        plt.plot(radial_history_iter_list, radial_history[:, r_i, 4], linestyle='-', color=colors[r_i])
    plt.legend([('R%d' % r_i) for r_i in range(0, radial_history.shape[1])])
    plt.grid(True)
    plt.xlim(max(iter_list) * -0.02, max(iter_list) * 1.02)
    plt.ylim(radial_history[:, :, -2:].min() - 0.01, radial_history[:, :, -2:].max() + 0.01)
...
def adapt(args):
    ...
    trainer = FinetuneRadialTrainer(cfg, is_base_model=False)
    assert isinstance(trainer._trainer, SimpleTrainer), 'trainer class mismatch'
    trainer._trainer.run_step = types.MethodType(radial_finetune_simple_trainer_run_step, trainer._trainer)
    assert isinstance(trainer.data_loader.dataset.dataset.dataset._map_func._obj, detectron2.data.DatasetMapper), 'mapper class mismatch'
    ...
    prefix = 'adapt%s_%s_anno_%s_radial%s' % (args.id, args.model, desc_pseudo_anno, '_mixup' if args.mixup_p > 0 else '')
    results_0 = {}
    for idx, dataset_name in enumerate(trainer.cfg.DATASETS.TEST):
        print('Evaluate on %s' % dataset_name)
        data_loader = trainer.build_test_loader(trainer.cfg, dataset_name)
...
class PredictorRadial(DefaultPredictor):
    def __init__(self, cfg):
        ...
        self.model = GeneralizedRCNNFinetuneRadial.create_from_sup(self.model)
        self.model.eval()
...
python finetune_radial_mixup.py --opt base --model r101-fpn-3x --cocodir ../../../MSCOCO2017 --num_workers 0 --iters 300 --eval_interval 101 --image_batch_size 2 --lr 0.01 --smallscale 1
python finetune_radial_mixup.py --opt base --model r101-fpn-3x --cocodir ../../../MSCOCO2017 --num_workers 4 --iters 40000 --eval_interval 3000 --image_batch_size 4

python finetune_radial_mixup.py --id 001 --opt adapt --model r101-fpn-3x --anno_models r50-fpn-3x r101-fpn-3x --ckpt mscoco2017_remap_radial_r101-fpn-3x.pth --cocodir ../../../MSCOCO2017 --num_workers 0 --iters 300 --eval_interval 101 --train_on_coco 1 --image_batch_size 2 --not_eval_coco 1 --lr 0.0001 --mixup_p 0

python finetune_radial_mixup.py --opt eval --model r101-fpn-3x --cocodir ../../../MSCOCO2017 --id 001 --ckpt F:\\intersections_results\\radial_r101\\adapt001_r101-fpn-3x_anno_refine_r101-fpn-3x_r50-fpn-3x_cocotrain_radial.pth

python finetune_radial_mixup.py --opt tp --id 001 --model r101-fpn-3x --ckpt mscoco2017_remap_radial_r101-fpn-3x.pth

python compare_baselines.py --opt compare --model r101-fpn-3x --arch radial --compare_ckpt_dir F:\\intersections_results\\cvpr24\\radial_r101
'''
