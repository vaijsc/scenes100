...
class GeneralizedRCNNServerCluster(GeneralizedRCNNServer):
    ...
    def update_clusters(self):
        # clustering algorithm: Feature based
    ...
    def get_training_assignment(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        if self.train_iter == 0: # first iter
    ...
    def set_buffer(self, buffer):
        self.video_input_buffer = buffer
...
def get_initial_buffer(dst_pseudo_anno, buffer_size, dataset_mapper, mode='random'):
    buffer = {}
    ...
    for idx in video_id_list:
        buffer[idx] = []
        for dataset_dict in dst_pseudo_anno:    
            if dataset_dict['video_id'] == idx:
                buffer[idx].append(dataset_dict)
        if mode == 'random':
            buffer[idx] = random.choices(buffer[idx], k=buffer_size)
        for i in range(buffer_size):
            buffer[idx][i] = dataset_mapper(buffer[idx][i])
    return buffer
...
def adapt(args):
    ...
    if args.incremental_videos:
        print("Online incremental video")
    ...
    # dataset_mapper = DatasetMapperPseudo.create_from_sup(trainer.data_loader.dataset.dataset.dataset._map_func._obj, cfg) 
    # initial_buffer = get_initial_buffer(copy.deepcopy(dst_pseudo_anno), args.buffer_size, dataset_mapper)
    ...
    dataset_mapper = DatasetMapperPseudo.create_from_sup(trainer.data_loader.dataset.dataset.dataset._map_func._obj, cfg) 
    initial_buffer = get_initial_buffer(copy.deepcopy(dst_pseudo_anno), args.buffer_size, dataset_mapper)
    trainer.data_loader.dataset.dataset.dataset._map_func._obj = dataset_mapper
    trainer.model.set_buffer(initial_buffer)
...
if __name__ == '__main__':
    ...
    print(args)
    print("Incremental videos: ", args.incremental_videos)
