...
class GeneralizedRCNNServerCluster(GeneralizedRCNNServer):
    ...
    def get_training_assignment(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        for im in batched_inputs:
            ...
            if im['video_id'] == 'coco':
            ...
            elif im['video_id'] not in self.video_id_to_index:
                loss_set = []
                ...
                while len(loss_set) > 0:
                    ...
                    if i_module in index_to_video_id:
                        index_to_video_id[i_module].append(im['video_id'])
                    else:
                        index_to_video_id[i_module] = [im['video_id']]
                    
                    self.video_id_to_index[im['video_id']] = i_module
                    self.used_indices[i_module] = True
                    if i_module in self.un_used_indices:
                        del self.un_used_indices[i_module]
                    break

                print(f'mapping (id {im["video_id"]} first seen in traning):')
                for i in sorted(list(index_to_video_id.keys())):
                    print(i, sorted(index_to_video_id[i]))
                print('unused:', self.un_used_indices)
                module_indices.append(self.video_id_to_index[im['video_id']])
...
python inference_server_simulate_greedy_assignment.py --train_whole 1 --opt adapt --model r101-fpn-3x --ckpt ../../models/mscoco2017_remap_r101-fpn-3x.pth --clustering 1 --update_cluster_interval 10005 --buffer_size 40 --incremental_videos 1 --tag seq.cluster.greedy.budget10 --budget 10 --iters 180000 --eval_interval 2000 --save_interval 2000 --image_batch_size 6 --num_workers 4 --p_replay 0.75 --outputdir ./greedy_fixbug
'''
