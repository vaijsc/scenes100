...
finetune_output = os.path.join(os.path.dirname(__file__), 'finetune_output_wdiff_latefusion_mixup')

from finetune import refine_annotations, get_annotation_dict, finetune_simple_trainer_run_step
from finetune_wdiff_earlyfusion import all_pseudo_manual_annotations_with_background, construct_image_w_background, DatasetMapperBackground
from finetune_wdiff_latefusion import GeneralizedRCNNFinetuneBackground, FinetuneBackgroundTrainer
...
def adapt(args):
    ...
    desc_cocovalid, dst_cocovalid = 'mscoco2017_valid_remap_wdiff_latefusion_mixup', get_coco_dicts(_args, 'valid')
    ...
    if args.id in video_id_list:
        ...
        desc_manual_valid, dst_manual_valid = '%s_manual_wdiff_latefusion_mixup' % args.id, get_annotation_dict(args)
        ...
        import functools
        args.id = '_compound'
        desc_manual_valid, desc_pseudo_anno = '%s_manual_wdiff_latefusion' % args.id, 'refine_' + '_'.join(args.anno_models)
    ...
    prefix = 'adapt%s_%s_anno_%s%s%s_mixup' % (args.id, args.model, desc_pseudo_anno, '' if args.fn_max_samples <= 0 else '_fn%.4f_%d' % (args.fn_min_score, args.fn_max_samples), '' if args.coco_inpaint_type == 'mask' else '_boxinpaint')
...
if __name__ == '__main__':
    ...
    if args.opt == 'adapt':
        adapt(args)
        # detectron2.engine.launch(adapt, 2, args=(args,), num_machines=1, machine_rank=0, dist_url='tcp://127.0.0.1:19999')
    elif args.opt == 'base':
        train_base(args)
        # detectron2.engine.launch(train_base, 2, args=(args,), num_machines=1, machine_rank=0, dist_url='tcp://127.0.0.1:19999')
    elif args.opt == 'eval':
        evaluate(args)
    else: pass
    exit(0)
