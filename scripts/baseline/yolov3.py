...
from yolov3_utils.parse_config import parse_model_config
from yolov3_utils.utils import *
from yolov3_utils.augmentations import *
from yolov3_utils.transforms import *
from yolov3_utils.loss import compute_loss
...
class Darknet(nn.Module):
    ...
    def reverse_yolo_transform(self, yolo_outputs, batched_inputs):
        ...
        for i, output in enumerate(yolo_outputs):
            ...
            resize_ratio = max([h_old, w_old]) / self.input_size
            pad = int((max([h_old, w_old]) - min([h_old, w_old]))) // 2
            boxes = []
            scores = []
            pred_classes = []
            for box in output:
                ...
                if h_old < w_old:
                    ...
                    x1 -= pad
                    x2 -= pad
                if not (0 <= x1 <= w_old and 0 <= y1 <= h_old and 0 <= x2 <= w_old and 0 <= y2 <= h_old):
                    continue
                boxes.append([x1, y1, x2, y2])
                scores.append(conf)
                pred_classes.append(cls_index)
    ...
    def preprocess_image(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        for i, im in enumerate(batched_inputs):
            ...
            if self.training:
                h_img, w_img = im['image'].shape[0], im["image"].shape[1]
                for j, instance in enumerate(im['instances'].gt_boxes.tensor):
                    # XYXY to relative XYWH
                    x1, y1, x2, y2 = instance[0], instance[1], instance[2], instance[3]
                    x = (x1 + x2) / 2
                    y = (y1 + y2) / 2
                    w = x2 - x1
                    h = y2 - y1
                    im['instances'].gt_boxes.tensor[j] = torch.Tensor([x/w_img, y/h_img, w/w_img, h/h_img])
                labels = im['instances'].gt_classes.reshape(-1, 1)
                instances = torch.cat([labels, im['instances'].gt_boxes.tensor], dim=1)
                # padding to square and scale image to [0, 1]
                im['image'], instances = AUGMENTATION_TRANSFORMS((im['image'].numpy(), instances.numpy()))
                instances[:, 0] = i
                im['instances'] = instances
            else:
                pad_transform = iaa.Sequential([
                                iaa.PadToAspectRatio(
                                    1.0,
                                    position="center-center").to_deterministic()])
                im['image'] = np.ndarray.astype(pad_transform(image=im['image'].numpy()), np.uint8)
                im['image'] = torchvision.transforms.ToTensor()(im['image'])
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        x = torch.stack([torchvision.transforms.functional.resize(im['image'], self.input_size) for im in batched_inputs]).cuda()
        targets = torch.cat([im['instances'] for im in batched_inputs], dim=0).cuda()
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            if module_def["type"] in ["convolutional", "upsample", "maxpool"]:
                ...
                x = layer_outputs[-1] + layer_outputs[layer_i]
                    ...
                    yolo_outputs.append(x)
            layer_outputs.append(x)
        outputs = non_max_suppression(torch.cat(yolo_outputs, 1), conf_thres=0.10, iou_thres=0.45)
...
def load_yolov3(model_path, weights_path=None):
...
if __name__ == '__main__':
    model = load_model('../config/yolov3-custom.cfg', '../adapt003_r101-fpn-3x_anno_train_003_refine_r101-fpn-3x_r50-fpn-3x_midfusion_mixup.pth')
    breakpoint()