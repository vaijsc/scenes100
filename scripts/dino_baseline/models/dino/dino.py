...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        if budget is not None:
            _class_embed = nn.ModuleList([copy.deepcopy(_class_embed) for _ in range(budget)])
            _bbox_embed = nn.ModuleList([copy.deepcopy(_bbox_embed) for _ in range(budget)])
    ...
    def forward(self, samples: NestedTensor, targets:List=None, module_indices=None, return_features=False):
        ...
        features, poss = self.backbone(samples, module_indices)
        if return_features:
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks, input_query_bbox, poss, input_query_label, attn_mask, module_indices)
        ...
        for dec_lid, (layer_ref_sig, layer_bbox_embed, layer_hs) in enumerate(zip(reference[:-1], self.bbox_embed, hs)):
            if isinstance(layer_bbox_embed, nn.ModuleList):
                layer_delta_unsig = torch.cat([layer_bbox_embed[b_id](layer_hs[j : j + 1]) for j, b_id in enumerate(module_indices)])
            else:
        ...
        for layer_cls_embed, layer_hs in zip(self.class_embed, hs):
            if isinstance(layer_cls_embed, nn.ModuleList):
                outputs_class.append(torch.cat([layer_cls_embed[b_id](layer_hs[j : j + 1]) for j, b_id in enumerate(module_indices)]))
            else:
        ...
        if hs_enc is not None:
            ...
            if isinstance(self.transformer.enc_out_class_embed, nn.ModuleList):
                interm_class = torch.cat([self.transformer.enc_out_class_embed[b_id](hs_enc[-1][j : j + 1]) for j, b_id in enumerate(module_indices)])
            else:
            ...
            if hs_enc.shape[0] > 1:
                ...
                for layer_id, (layer_box_embed, layer_class_embed, layer_hs_enc, layer_ref_enc) in enumerate(zip(self.enc_bbox_embed, self.enc_class_embed, hs_enc[:-1], ref_enc[:-1])):
                    if isinstance(layer_box_embed, nn.ModuleList):
                        layer_enc_delta_unsig = torch.cat([layer_box_embed[b_id](layer_hs_enc[j : j + 1]) for j, b_id in enumerate(module_indices)])
                    else:
                    ...
                    if isinstance(layer_class_embed, nn.ModuleList):
                        layer_enc_outputs_class = torch.cat([layer_class_embed[b_id](layer_hs_enc[j : j + 1]) for j, b_id in enumerate(module_indices)])
                    else:
...
def build_dino(args):
    ...
    try:
        ...
        dec_pred_bbox_embed_share = True
    # args.budget = None # do not split head
    model = DINO(
        backbone,
        transformer,
