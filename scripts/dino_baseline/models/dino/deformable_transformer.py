...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, module_indices=None):
        """
        Input:
        ...
        if self.two_stage_type =='standard':
            ...
            if isinstance(self.enc_out_class_embed, nn.ModuleList):
                enc_outputs_class_unselected = torch.cat([self.enc_out_class_embed[b_id](output_memory[j : j + 1]) for j, b_id in enumerate(module_indices)])
            else:
                enc_outputs_class_unselected = self.enc_out_class_embed(output_memory)
            if isinstance(self.enc_out_bbox_embed, nn.ModuleList):
                enc_outputs_coord_unselected = torch.cat([self.enc_out_bbox_embed[b_id](output_memory[j : j + 1]) for j, b_id in enumerate(module_indices)]) + output_proposals # (bs, \sum{hw}, 4) unsigmoid
            else:
    ...
    def forward(self, 
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if ((layer_id == 0 and self.two_stage_type in ['enceachlayer', 'enclayer1']) \
                ...
                # gather boxes
                topk = self.num_queries
                if isinstance(self.class_embed[layer_id], nn.ModuleList):
                    enc_outputs_class = torch.cat([self.class_embed[layer_id][b_id](output_memory[j : j + 1]) for j, b_id in enumerate(module_indices)])
                else:
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                if isinstance(self.bbox_embed[layer_id], nn.ModuleList):
                    delta_unsig = torch.cat([self.bbox_embed[layer_id][b_id](output[:, j : j + 1, :]) for j, b_id in enumerate(module_indices)], dim=1)
                else:
                ...
                if self.dec_layer_number is not None and layer_id != self.num_layers - 1:
                    ...
                    if nq_now != select_number:
                        if isinstance(self.class_embed[layer_id], nn.ModuleList):
                            class_unselected = torch.cat([self.class_embed[layer_id][b_id](output[:, j : j + 1, :]) for j, b_id in enumerate(module_indices)], dim=1)
                        else:
