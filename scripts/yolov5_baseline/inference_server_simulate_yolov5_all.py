...
class YOLOServer(DetectionModel):
    ...
    def merge_detection_output(self, outputs, train=True):
        if train:
            out = []
            for i in range(len(outputs[0])):
                out.append(torch.cat([xi[i] for xi in outputs], dim=0))
            return out
        else:
            out = []
            out.append(torch.cat([xi[0] for xi in outputs], dim=0))
            out2 = []
            for i in range(len(outputs[0][1])):
                out2.append(torch.cat([xi[1][i] for xi in outputs], dim=0))
            out.append(out2)
            return out
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], augment=False, profile=False, visualize=False):
        ...
        x, targets = self.preprocess_image(batched_inputs)
        # end_pre = time.perf_counter()
        # print("preprocess_time: ", end_pre - start_time)
        # print("input_size: ", x[0].shape)
        ...
        for i, m in enumerate(self.model):
            ...
            if i in self.split_list: # split on 2 first Conv layers + 1 C3 block and Detection head
                if type(x) != list:
                    x = [m[b_id](x[j : j + 1]) for j, b_id in enumerate(module_indices)]
                else:
                    out = []
                    for j, b_id in enumerate(module_indices):
                        out.append(m[b_id]([xi[j: j + 1] for xi in x]))
                    x = out

                if type(x[0]) is list or type(x[0]) is tuple: # Detection head output
                    x = self.merge_detection_output(x)             
                else:
                    x = torch.cat(x, dim=0)
            else:
                x = m(x)  # run
        ...
        # yolo_outputs = x
        loss, loss_components = self.compute_loss(x, targets)
        # print("one forward pass time: ", time.perf_counter() - end_pre)
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]], augment=False, profile=False, visualize=False, feature=False):
        ...
        # find module ID for each sample in inference mode
        module_indices = []
        for im in batched_inputs:
        ...
        # x_clone = x.clone()
        # ratio = batched_inputs[0]['image'].shape[-2] / batched_inputs[0]['height']
        ...
        for i, m in enumerate(self.model):
            ...
            if i in self.split_list: # split on 2 first Conv layers + 1 C3 block and Detection head
                ...
                if type(x[0]) is list or type(x[0]) is tuple: # Detection head output
                    x = self.merge_detection_output(x, train=False)             
                else:
                    x = torch.cat(x, dim=0)
            else:
                x = m(x)  # run
        ...
        outputs = non_max_suppression(x[0], conf_thres=0.001, iou_thres=0.45)
        ...
        # breakpoint()
        # DetectionModel.draw(x_clone[0], results[0]['instances'], format='frcnn', ratio=ratio)
        # breakpoint()
        return results
    ...
    def create_from_sup(cls, net, budget, split_list):
        ...
        for i, m in enumerate(net.model):
            if i in net.split_list: # split on 2 first Conv layers + 1 C3 block and Detection head
                net.model[i] = torch.nn.ModuleList([deepcopy(m) for _ in range(net.budget)]) 
 
        net.__class__ = cls
        return net

class YOLOServerCluster(YOLOServer):
    def buffering_with_replay(self, batched_inputs: List[Dict[str, torch.Tensor]]):
        ...
        if np.random.rand() < self.p_buffer:
            ...
            if len(self.video_input_buffer[v_buffer]) < self.buffer_size:
                self.video_input_buffer[v_buffer].append(deepcopy(batched_inputs[i_im]))
            else:
                self.video_input_buffer[v_buffer][np.random.choice(np.arange(0, len(self.video_input_buffer[v_buffer])))] = deepcopy(batched_inputs[i_im])
        ...
        if np.random.rand() < self.p_replay and len(self.video_input_buffer) > 0:
            ...
            batched_inputs[i_im] = deepcopy(np.random.choice(self.video_input_buffer[v_replay]))
    ...
    def update_clusters(self):
        ...
        for i_module in tqdm.tqdm(range(0, self.budget), ascii=True, desc='calculating for each module'):
            for v in self.video_input_buffer:
                _losses_list = []
                for im in self.video_input_buffer[v]:
                    with torch.no_grad():
                        _losses_list.append(self.forward([im])[0].item())
                video_index_loss_set.append((v, i_module, sum(_losses_list) / len(_losses_list)))
        ...
        while len(video_index_loss_set) > 0:
            ...
            if len(index_to_video_id[i_module]) >= _cap_per_module:
                continue
    ...
    @classmethod
    def create_from_sup(cls, net, budget, split_list, cluster_cap_factor=1.5, buffer_size=10, update_cluster_interval=100, p_buffer=0.1, p_replay=0.25):
        assert cluster_cap_factor > 0.01
        assert buffer_size >= 1
        assert update_cluster_interval > buffer_size
        assert p_buffer > 0.01
        assert p_replay > 0.01
        net = YOLOServer.create_from_sup(net, budget, split_list)
...
class AdaptativePartialTrainer(DefaultTrainer):
    def __init__(self, cfg, train_whole, incremental_videos, clustering, ckpt):
        ...
        model = load_yolov5(os.path.join(os.path.dirname(__file__), cfg.YOLO_CONFIG_PATH), ckpt)

        # self.model_teacher = deepcopy(model)
        self.model_teacher = load_yolov5(os.path.join(os.path.dirname(__file__), cfg.YOLO_CONFIG_PATH), "../../models/yolov5s_remap.pth")
        self.model_teacher.output_format = "yolo"
        self.model_teacher.eval()
        if clustering:
            model = YOLOServerCluster.create_from_sup(model, cfg.MODEL.ADAPTIVE_BUDGET, cfg.MODEL.SPLIT_LIST, cfg.MODEL.CLUSTER_CAP_FACTOR, cfg.MODEL.BUFFER_SIZE, cfg.MODEL.UPDATE_CLUSTER_INTERVAL, cfg.MODEL.P_BUFFER, cfg.MODEL.P_REPLAY)
        else:
...
def finetune_ema_simple_trainer_run_step(self):
    ...
    if len(pseudo_idx) > 0:
        with torch.no_grad():
            ...
            for _i, _pred in zip(pseudo_idx, pseudo_labels):
                # _mask = _pred[:, 4] >= self.pseudo_det_min_score
                # _filtered = _pred[_mask]
    ...
    loss_dict_items =   {"IoU loss": float(loss_components[0]),
                        "Object loss": float(loss_components[1]),
                        "Class loss": float(loss_components[2])}
    loss_dict =   {"IoU loss": loss_components[0],
                        "Object loss": loss_components[1],
                        "Class loss": loss_components[2]}
...
def adapt(args):
    ...
    prefix = 'adaptive_partial_server_%s_anno_%s.%s' % ("yolov3", desc_pseudo_anno, args.tag)
    ...
    trainer = AdaptativePartialTrainer(cfg, args.train_whole, args.incremental_videos, args.clustering, args.ckpt)
    ...
    #             state_dict['stem_res2_list.%d.0.%s' % (i, k[24:])] = deepcopy(state_dict[k])
    #     if k.startswith('backbone.bottom_up.res2.'):
    #         for i in range(0, args.budget):
    #             state_dict['stem_res2_list.%d.1.%s' % (i, k[24:])] = deepcopy(state_dict[k])
    #     if k.startswith('proposal_generator.'):
    #         for i in range(0, args.budget):
    #             state_dict['proposal_generator_list.%d.%s' % (i, k[19:])] = deepcopy(state_dict[k])
    #     if k.startswith('roi_heads.box_predictor.'):
    #         for i in range(0, args.budget):
    #             state_dict['box_predictor_list.%d.%s' % (i, k[24:])] = deepcopy(state_dict[k])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_seed', type=int, nargs='+') #TODO: two args seed
