...
class YOLOServer(DetectionModel):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor]], augment=False, profile=False, visualize=False):
        ...
        for i, m in enumerate(self.model):
            ...
            if i in [0, 1, 2] or i == len(self.model) - 1: # split on 2 first Conv layers + 1 C3 block and Detection head
                if type(x) != list:
    ...
    def inference(self, batched_inputs: List[Dict[str, torch.Tensor]], augment=False, profile=False, visualize=False):
        ...
        # breakpoint()
        # DetectionModel.draw(x_clone[0], results[0]['instances'], format='frcnn', ratio=ratio, file_name="output_student.png")
        # breakpoint()
        return results
    ...
    def create_from_sup(cls, net, budget):
        ...
        for i, m in enumerate(net.model):
            if i in [0, 1, 2] or i == len(net.model) - 1: # split on 2 first Conv layers + 1 C3 block and Detection head
...
class YOLOServerCluster(YOLOServer):
    ...
    def create_from_sup(cls, net, budget, cluster_cap_factor=1.5, buffer_size=10, update_cluster_interval=100, p_buffer=0.1, p_replay=0.25):
        ...
        net = YOLOServer.create_from_sup(net, budget)
...
class AdaptativePartialTrainer(DefaultTrainer):
    def __init__(self, cfg, train_whole, incremental_videos, clustering, ckpt):
        ...
        if clustering:
            model = YOLOServerCluster.create_from_sup(model, cfg.MODEL.ADAPTIVE_BUDGET, cfg.MODEL.CLUSTER_CAP_FACTOR, cfg.MODEL.BUFFER_SIZE, cfg.MODEL.UPDATE_CLUSTER_INTERVAL, cfg.MODEL.P_BUFFER, cfg.MODEL.P_REPLAY)
        else:
            model = YOLOServer.create_from_sup(model, cfg.MODEL.ADAPTIVE_BUDGET)
        # model = model.cuda()
        # self.model_teacher = self.model_teacher.cuda()
        if train_whole:
            trainable_modules = [model]
        else:
            trainable_modules = [model.model[i] for i in [0, 1, 2, -1]]
...
if __name__ == '__main__':
    ...
    print(args)
    assert args.id != '', 'Video ID not provided!'
