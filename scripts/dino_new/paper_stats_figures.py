...
video_id_list = ['001', '003', '005', '006', '007']
...
def ema_ablation():
    ...
    apgs = [
        ('EMA teacher', '#00AAAA', [0,   5.82,   4.47,   1.82,  -0.65,  -2.57,  -3.91,  -4.26,  -3.99,  -5.02],
                                   [0,   7.67,   5.40,   1.67,  -1.25,  -3.33,  -5.03,  -5.90,  -5.88,  -7.55], '^', '-'),
        ('EMA student', '#006666', [0,   5.16,   2.78,  -0.17,  -2.11,  -3.63,  -4.93,  -4.62,  -4.47,  -5.78],
...
def moe_ablation():
    ...
    iters = [100000, 180000]
    ylims = [43.7, 45.65]
    yticks = [44, 44.5, 45, 45.5]
    plt.figure(figsize=figsize)
    plt.plot(iters, np.array([8.7179, 8.96]) + baseline_mAP, marker='o', ls='-', color='#000000', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 9.65]) + baseline_mAP, marker='^', ls='-', color='#AA0000', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 9.86]) + baseline_mAP, marker='D', ls='-', color='#0000AA', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 8.95]) + baseline_mAP, marker='^', ls='--',color='#000077', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 9.13]) + baseline_mAP, marker='D', ls='--',color='#004444', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.legend(['1-for-100', '$B$=10, proposed MoE', '$B$=100, proposed MoE', '$B$=10, intermediate MoE', '$B$=100, intermediate MoE'], loc='upper left')
    plt.xlim(min(iters) - 10000, max(iters) + 10000)
    plt.ylim(*ylims)
    plt.xticks(iters)
    plt.yticks(yticks)
    ax = plt.gca()
    ...
    plt.plot(iters, np.array([8.7179, 9.65]) + baseline_mAP, marker='^', ls='-', color='#AA0000', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 9.86]) + baseline_mAP, marker='D', ls='-', color='#0000AA', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.3282, 8.88]) + baseline_mAP, marker='^', ls='--',color='#000077', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.2496, 8.65]) + baseline_mAP, marker='D', ls='--',color='#004444', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    ...
    plt.plot(iters, np.array([8.7179, 8.96]) + baseline_mAP, marker='o', ls='-', color='#000000', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 9.65]) + baseline_mAP, marker='^', ls='-', color='#AA0000', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.7179, 9.20]) + baseline_mAP, marker='^', ls='--',color='#000077', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.3282, 8.88]) + baseline_mAP, marker='^', ls=':',color='#442266', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
    plt.plot(iters, np.array([8.0914, 8.66]) + baseline_mAP, marker='^', ls='-.',color='#224455', lw=3, ms=9, markerfacecolor='none', markeredgewidth=2)
...
def oracle_AP():
    descs = ['Base\nmodel', 'Proposed\n$B$=10', 'Proposed\n$B$=100', 'Oracle\nmodel']
    colors= ['#333333',    '#BB3300',         '#BB0033',          '#0000FF']
    APs = {
        'frcnn_r18':  (np.array([35.82, 45.54, 45.36, 50.60]), [40, 45, 50]),
        'frcnn_r101': (np.array([41.96, 50.22, 50.35, 54.77]), [45, 50, 55]),
        'yolov8s':    (np.array([44.13, 46.41, 46.37, 57.80]), [45, 50, 55]),
        'dino':       (np.array([40.72, 43.71, 43.46, 48.66]), [42, 44, 46, 48]),
    }
    for m, (ap_m, yticks) in APs.items():
        fig, ax1 = plt.subplots(figsize=(5, 5))
        ax1.bar(range(0, len(descs)), ap_m, width=0.75, align='center', color=colors, linewidth=0, tick_label=descs)
        ax1.set_xlim(-0.5, len(descs) - 0.375)
        ylims = (ap_m.max() - ap_m.min()) / 20
        ylims = [ap_m.min() - ylims, ap_m.max() + ylims]
        ax1.set_ylabel('$AP^m$', labelpad=3)
        ax1.set_ylim(*ylims)
        ax1.set_yticks(yticks)
        ax1.grid(True)
        ax2 = ax1.twinx()
        ax2.set_ylabel('$APD^m$', labelpad=1)
        ax2.set_ylim(*ylims)
        ap_m = np.concatenate([ap_m[:2], ap_m[-1:]]) # only show B=10
        ax2.set_yticks(ap_m)
        ax2.set_yticklabels(['%.2f' % (x - ap_m[0]) for x in ap_m])
        fig.tight_layout()
        # plt.show()
        plt.savefig('oracle_%s.pdf' % m)
...
def case_study_MoE():
    ...
    outputdir = '.'
    with open(os.path.join(outputdir, 'result13567_split.json'), 'r') as fp:
    ...
    for v in ['001', '003', '005', '006', '007']:
        ...
        with open(os.path.join(inputdir, 'annotations.json'), 'r') as fp:
            annotations = json.load(fp)
        for im in tqdm.tqdm(dets_base, ascii=True):
            annotations = list(filter(lambda x: x['score'] > t_base[x['category_id']], im['annotations']))
            image = skimage.io.imread(os.path.join(inputdir, 'unmasked', im['file_name']))
            skimage.io.imsave(os.path.join(outputdir, 'samples', '%s_%s_base.jpg' % (v, im['file_name'][:-4])), draw_bbox(image, annotations, 'base model'), quality=80)
