...
class MakeMoE(torch.nn.Module):
    ...
    #     out = [self.experts[m].weight[:, None, :] for m in self.module_indices]
    #     self.module_indices = None
    #     return torch.cat(out, dim=1).transpose(0, 1)    


class MakeMoEQueries(torch.nn.Module):
    def __init__(self, net, budget, generic_query_ratio=0):
        super(MakeMoEQueries, self).__init__()
        weight = net.weight
        num_queries, d_model = weight.shape[0], weight.shape[1]
        num_generic_queries = int(generic_query_ratio*num_queries)
        num_specific_queries = num_queries - num_generic_queries
        self.experts = []
        self.generic_embedding = torch.nn.Embedding(num_generic_queries, d_model)
        self.generic_embedding.weight.data = weight[:num_generic_queries, :]
        for _ in range(budget):
            specific_embedding = torch.nn.Embedding(num_specific_queries, d_model)
            specific_embedding.weight.data = weight[-num_specific_queries:, :]
            self.experts.append(specific_embedding)
        self.experts = torch.nn.ModuleList(self.experts)
    def get_weights(self):
        generic_weights = self.generic_embedding.weight[:, None, :].repeat(1, len(self.module_indices), 1).transpose(0, 1)
        specific_weights = torch.cat([self.experts[m].weight[:, None, :] for m in self.module_indices], dim=1).transpose(0, 1)
        self.module_indices = None
        return torch.cat([generic_weights, specific_weights], dim=1)
...
class DINOServer(Dino):
    ...
    def forward(self, batched_inputs, return_features=False):
        ...
        # assign module indices to submodule.
        ...
        # self.model.transformer.tgt_embed.module_indices = module_indices
    ...
    @classmethod
    def create_from_sup(cls, net, budget, generic_query_ratio=0):
        ...
        # # net.model.transformer.tgt_embed = MakeMoEQueries(net.model.transformer.tgt_embed, budget, generic_query_ratio)
...
class AdaptativePartialTrainer(DefaultTrainer):
    def __init__(self, cfg, incremental_videos, ckpt):
        ...
        model = DINOServer.create_from_sup(model, cfg.MODEL.ADAPTIVE_BUDGET)
...
def adapt(args):
    ...
    cfg.SOLVER.GENERIC_QUERY_RATIO = args.generic_query_ratio
...
if __name__ == '__main__':
    ...
    parser.add_argument('--generic_query_ratio', type=float, default=0)
